(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))c(i);new MutationObserver(i=>{for(const s of i)if(s.type==="childList")for(const r of s.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&c(r)}).observe(document,{childList:!0,subtree:!0});function o(i){const s={};return i.integrity&&(s.integrity=i.integrity),i.referrerPolicy&&(s.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?s.credentials="include":i.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function c(i){if(i.ep)return;i.ep=!0;const s=o(i);fetch(i.href,s)}})();function B(){const e=document.querySelector("#hero");e.innerHTML=`
  ...
  `;const t=document.querySelector("#contactForm"),o=document.querySelector("#contactStatus"),c="https://formspree.io/f/xvgvayno";let i=null;function s(r,d="success",m=5e3){if(!o)return;i&&(clearTimeout(i),i=null),o.innerHTML=`
    ...
    `,o.querySelector(".notification");const l=o.querySelector(".notification__close");function p(){o&&(o.innerHTML="",i&&(clearTimeout(i),i=null))}l&&l.addEventListener("click",p),m>0&&(i=setTimeout(p,m))}t.addEventListener("submit",async r=>{r.preventDefault();const d=t.querySelector('button[type="submit"]'),m=d.textContent;d.textContent="Sending...",d.disabled=!0;const l=new FormData(t),p=Object.fromEntries(l.entries());try{const y=await fetch(c,{method:"POST",headers:{Accept:"application/json","Content-Type":"application/json"},body:JSON.stringify(p)});if(y.ok)s("Thank you — your message has been sent!","success",6e3),t.reset();else{const u=await y.json().catch(()=>null);console.error("Formspree error",y.status,u),s("There was a problem sending your message. Please try again later.","error",8e3)}}catch(y){console.error("Network error while sending form",y),s("Network error. Please check your connection and try again.","error",8e3)}finally{d.textContent=m,d.disabled=!1}})}function $(){document.querySelectorAll("main > section").forEach(e=>{e.style.display="none"}),document.querySelector("#contact").style.display="block",_()}function G(){const e=document.querySelector("#notFound");e&&(e.innerHTML=`
  ...
  `)}function U(){G()}function K(){const e=document.querySelector("#privacy");e.innerHTML=`
  ...
  `;const o=document.querySelector("#product-grid"),c=document.querySelector("#productSearch"),i=document.querySelectorAll(".filter-btn");let s="All",r="";function d(){const n=J.filter(a=>{const h=s==="All"||a.category===s,g=a.name.toLowerCase().includes(r)||a.desc.toLowerCase().includes(r)||a.category.toLowerCase().includes(r);return h&&g});if(n.length===0){o.innerHTML=`
    ...
    `).join(""),m()}function m(){document.querySelectorAll(".product-action-btn").forEach(n=>{n.addEventListener("click",a=>{const h=a.target.getAttribute("data-name"),g=a.target.getAttribute("data-price"),v=document.querySelector("#orderItems"),w=document.querySelector("#orderQuantity");if(v&&w){v.value=`Requested Product: ${h}
Unit Price: KES ${parseInt(g).toLocaleString()}`,w.value="1";const b=document.querySelector("#order-form-section");if(b){b.scrollIntoView({behavior:"smooth"});const f=b.querySelector(".urgent-order-card");f&&(f.classList.add("highlight-pulse"),setTimeout(()=>{f.classList.remove("highlight-pulse")},2e3))}}})})}c.addEventListener("input",n=>{r=n.target.value.toLowerCase().trim(),d()}),i.forEach(n=>{n.addEventListener("click",()=>{i.forEach(a=>a.classList.remove("active")),n.classList.add("active"),s=n.getAttribute("data-category"),d()})}),d(),Z();const l=document.querySelector("#urgentOrderForm"),p=document.querySelector("#orderStatus"),y="https://formspree.io/f/xvgvayno";let u=null;function S(n,a="success",h=5e3){if(!p)return;u&&(clearTimeout(u),u=null),p.innerHTML=`
      <div class="notification notification--${a}" role="status">
        <div class="notification__content">${n}</div>
        <button class="notification__close" aria-label="Dismiss notification">&times;</button>
      </div>
    `,p.querySelector(".notification");const g=p.querySelector(".notification__close");function v(){p&&(p.innerHTML="",u&&(clearTimeout(u),u=null))}g&&g.addEventListener("click",v),h>0&&(u=setTimeout(v,h))}l.addEventListener("submit",async n=>{n.preventDefault();const a=l.querySelector('button[type="submit"]'),h=a.textContent;a.textContent="Sending Request...",a.disabled=!0;const g=new FormData(l),v=g.get("phone"),w=g.get("company")||"N/A",b=g.get("urgency"),f=g.get("quantity"),F=g.get("location"),D=g.get("message"),A={name:g.get("name"),email:g.get("email"),subject:`Urgent Hardware Sourcing Request: ${b}`,company:w,phone:v,urgency:b,quantity:f,location:F,message:`
